...
export interface ProductsSectionBaseProps {
  theme: ThemeParams;
  content: ProductsServicesContent;
  variant?: 'alternating' | 'grid' | 'featured';
}

const ProductsSectionBase = ({ theme, content, variant }: ProductsSectionBaseProps) => {
  ...
  const productVariant = variant || content.variant || 'alternating';
  const productItems = content.items || [];

  const navigateToProduct = (item: ProductItem) => {
    if (!item.id) return;
    ...
    window.location.href = `/products/${toSlug(titleForSlug)}`
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {productItems.length > 0 && productVariant === 'grid' && (
          <div id="products" className="mt-20 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {productItems.map((item, index) => (
              <Card
                key={item.id || index}
                className={cn("h-full overflow-hidden border-border/50 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-elegant", getBorderRadiusClass())}
              >
                <div className="relative h-64 w-full">
                  <Image
                    src={item.image || "https://placehold.co/600x400?text=Product+Image"}
                ...
                <CardContent className="flex h-full flex-col p-6">
                  {item.category && (
                    <div className="mb-3 text-sm font-semibold uppercase tracking-wide" style={{ color: theme.colors.primary }}>
                      {item.category}
                    </div>
                  )}
                  <h3 className="text-2xl font-bold" style={{ color: content.textColor || theme.colors.text }}>
                    {item.name}
                  </h3>
                  <p className="mt-3 flex-1 leading-relaxed" style={{ color: content.textColor || theme.colors.muted || '#718096' }}>
                    {item.description}
                  </p>
                  {item.price && (
                    <div className="mt-5 text-xl font-bold" style={{ color: theme.colors.primary }}>
                      {item.price}
                    </div>
                  )}
                  {item.id && (
                    <Button className="mt-6 w-full" style={getButtonStyles('primary')} onClick={() => navigateToProduct(item)}>
                      View Details
        ...
        {productItems.length > 0 && productVariant === 'featured' && (
          <div id="products" className="mt-20">
            {productItems[0] && (
              <div className={cn("grid gap-8 overflow-hidden border bg-white p-5 shadow-elegant lg:grid-cols-[1.1fr_0.9fr]", getBorderRadiusClass())}>
                <div className="relative min-h-[360px]">
                  <Image
                    src={productItems[0].image || "https://placehold.co/800x560?text=Featured+Product"}
                    alt={productItems[0].name || "Featured product"}
                    fill
                    className={cn("object-cover", getBorderRadiusClass())}
                  />
                </div>
                <div className="flex flex-col justify-center p-3 lg:p-6">
                  <p className="text-sm font-semibold uppercase tracking-wide" style={{ color: theme.colors.primary }}>
                    Featured
                  </p>
                  <h3 className="mt-4 text-4xl font-bold leading-tight" style={{ color: content.textColor || theme.colors.text }}>
                    {productItems[0].name}
                  </h3>
                  <p className="mt-4 text-lg leading-8" style={{ color: content.textColor || theme.colors.muted || '#718096' }}>
                    {productItems[0].description}
                  </p>
                  {productItems[0].price && (
                    <div className="mt-5 text-2xl font-bold" style={{ color: theme.colors.primary }}>
                      {productItems[0].price}
                    </div>
                  )}
                  {productItems[0].id && (
                    <Button className="mt-7 w-fit" style={getButtonStyles('primary')} onClick={() => navigateToProduct(productItems[0])}>
                      View Details
                      <ArrowRight className="ml-2 h-5 w-5" />
                    </Button>
                  )}
            ...
            {productItems.length > 1 && (
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                {productItems.slice(1).map((item, index) => (
                  <Card key={item.id || index} className={cn("border-border/50 bg-white p-5", getBorderRadiusClass())}>
                    <h3 className="text-xl font-bold" style={{ color: content.textColor || theme.colors.text }}>
                      {item.name}
                    </h3>
                    <p className="mt-2 text-sm leading-6" style={{ color: content.textColor || theme.colors.muted || '#718096' }}>
                      {item.description}
                    </p>
                  </Card>
                ))}
        ...
        {productItems.length > 0 && productVariant === 'alternating' && (
          <div id="products" className="mt-20">
            {productItems.map((item, index) => (
...
export default ProductsSectionBase;
