...
class AILoadBalancer {
  ...
  selectBestKey(): string | never {
    ...
    const availableKeys = this.apiKeys.filter(keyInfo => {
      return keyInfo.errors < 50 && // Increased from 10 to 50 errors before skipping
        keyInfo.successRate > 0.5 && // Success rate above 50%
        (Date.now() - keyInfo.lastUsed) > 500 // Reduced from 1s to 500ms
    })
