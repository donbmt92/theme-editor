...
export function isQueueHealthy(): boolean {
  ...
  // Refined health check logic:
  // - Queue is healthy if queued tasks are within limits
  // - Average wait time is acceptable
  // - Failure rate is only considered significant after a minimum number of tasks (e.g., 20)
  //   to avoid blocking the queue due to a few early failures.
  const isHealthy = stats.queuedTasks < 4000 && 
                    avgWaitTimeMinutes < 5 && 
                    (totalTasks < 20 || failureRate < 0.25)
