...
async function processDeployOptimized({ deployData, userId, projectId, startTime }: {
  ...
  try {
    ...
    // Step 4: Write files in parallel chunks (optimal I/O)
    const CHUNK_SIZE = 5 // Write 5 files at a time
    const chunks = []
    ...
    for (const chunk of chunks) {
      await Promise.all(
        chunk.map(async (file) => {
          const content = await generateFileContent(file, themeParams, projectName, description, serverType, domain)
    ...
    // Minimal logging - chỉ log summary
    console.log(`✅ [DEPLOY] ${projectName} completed: ${fileManifest.length} files in ${totalTime}ms`)
...
async function generateFileContent(fileInfo: any, themeParams: any, projectName: string, description: string, serverType?: string, domain?: string): Promise<string> {
  ...
  if (fileInfo.type === 'placeholder') {
    ...
    switch (fileInfo.template) {
      ...
      case 'readme':
        ...
        break
      case 'deploy-script':
        content = generateDeployScript_func(projectName, fileInfo.serverType, domain)
        break
      default:
        content = ''
    }
...
// Background cleanup for old deploys
          ...
          if (entry.isDirectory()) {
            ...
            if (timestampMatch) {
              ...
              if (now - timestamp > MAX_DEPLOY_AGE) {
                ...
                console.log(`🧹 [CLEANUP] Removed old deploy: ${entryPath}`)
              }
            } else {
